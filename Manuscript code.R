...
# load filtered data
fox_data <- read.csv("fox data 2018-2025.csv")
fox_data$ID <- as.factor(fox_data$ID)
fox_data$GPS.Fix.Time <- as.POSIXct(fox_data$timestamp, format = "%Y.%m.%d %H:%M:%S")
fox_data <- fox_data %>% select(-GPS.Fix.Time)
...
# Example:
month_data <- fox_data %>%
  filter(month(timestamp) %in% c(1))   # 1 = January. Repeat for all twelve months.
...
ggplot(bc, aes(x = AKDE90_diff, y = overlap)) +
  geom_point() +  # Add the data points
  geom_smooth(method = "lm") +  # Add the regression line
  labs(title = "Regression of overlap on AKDE90_diff", 
       x = "AKDE90_diff", y = "Overlap") +
  theme_minimal()
...
bc2 %>%
  ...
  summarise(mean_overlap = mean(overlap, na.rm = TRUE),
            ...
            n = n())
ggplot(bc2, aes(x = sex, y = overlap, fill = sex)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +  # Boxplot with transparency
  geom_jitter(width = 0.2, alpha = 0.7) +  # Jitter to show individual points
  theme_minimal() +
  labs(x = "Sex", y = "Overlap", title = "Comparison of Overlap by Sex") +
  scale_fill_manual(values = c("Male" = "#1f78b4", "Female" = "#e31a1c"))  # Custom colors
